 ...
 *   gateway   removed in this install; TypeSafe only
...
export const DEFAULT_BASE_URL: Record<Provider, string> = {
  ...
  gateway: '', // removed: this install never uses the Vercel Gateway
}
...
export function selectProvider(
  ...
  // The Vercel AI Gateway is disabled. Only
  // TypeSafe or the built-in classifier can ever be selected, so no request
  // can reach the Gateway regardless of what the config asks for.
  void gatewayKey
  if (forced === 'builtin' || forced === 'gateway') return null
  return typesafeKey ? 'typesafe' : null
}
...
export function pendingDecisions(): {
  ...
  return {
    // Keep only the LATEST classification, never discard both.
    // jev classifies every prompt.submit, including prompts that never start a
    // turn: slash commands (/exit, /reload-skills) and interrupted turns.
    // Counting how many were waiting and throwing both away on finding two
    // would let one stray classification leave the NEXT real prompt
    // unrouted ("main loop: no decision"). Overwriting means a stale
    // classification is replaced by the real prompt's before the turn starts.
    put(decision) {
      held = decision
    },
    take() {
...
export function describeStatus(
  ...
  // Lead with the model the turn actually runs on, not the name
  // of the thing that decided it. "jev ·" said who chose; it never said what
  // you got, and on the `unchanged` path it named no model at all.
  const model = change?.model ?? current?.model ?? 'model unknown'
  const effort = change?.effort ?? current?.effort
  const on = `${model}${effort === undefined ? '' : `/${effort}`}`
  if (!decision) return `${on} · no decision`
  const asked = `${decision.tier} ${reported(decision.confidence)}`
  return change ? `${on} · ${asked} · routed` : `${on} · ${asked} · kept`
}
