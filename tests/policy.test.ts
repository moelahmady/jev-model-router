...
test('the latest classification wins, so a stray prompt does not unroute the next turn', () => {
  // jev classifies every prompt.submit, including slash commands and
  // interrupted turns that never start one. Discarding BOTH on seeing two
  // would make the next real prompt run unrouted.
  const pending = pendingDecisions()
...
test('a failed classification is overwritten by the next prompt, not sticky', () => {
  const pending = pendingDecisions()
  pending.put(null)
  pending.put(deepDecision)
  expect(pending.take()).toEqual(deepDecision)
...
test('a failed classification after a good one clears the slot', () => {
  const pending = pendingDecisions()
  pending.put(deepDecision)
  pending.put(null)
...
test('the slot recovers after a cleared round', () => {
  const pending = pendingDecisions()
  pending.put(deepDecision)
  pending.put(null)
  expect(pending.take()).toBeNull()
...
test('the status line names the real model, and marks routed vs kept', () => {
  // Leading with "jev" and naming no model on the no-change path would make
  // "did it switch?" unanswerable.
  const decision = readDecision(gatewayAnswer('fast', { fast: 0.87 }))
  const current = { model: 'claude-opus-5', effort: 'medium' }
  expect(describeStatus(decision, { model: 'haiku', effort: 'low' }, current)).toBe(
    'haiku/low · fast 0.87 · routed',
  )
  expect(describeStatus(decision, null, current)).toBe('claude-opus-5/medium · fast 0.87 · kept')
  expect(describeStatus(null, null, current)).toBe('claude-opus-5/medium · no decision')
})
